...
- New `docs/implementers/conformance-checklist.md` separating the machine-checked surface (`npm test`) from the transport-layer, runtime-policy, and cross-call requirements implementers MUST self-attest.
...
- New `SPEC.md` Conformance Requirement (cross-profile correlation): servers MUST NOT rely on per-profile narrowness alone to prevent inference across multiple authorized calls; servers granting multi-profile or repeat single-profile access for the same requester SHOULD apply per-requester rate limits across profiles, correlation-aware authorization logging, and per-request minimization. Surfaces an unresolved design question into the normative document.
- New `docs/implementers/compatibility-risks.md` §Decisions Needed Before 1.0 §Cross-profile inference controls section enumerating four open sub-decisions (correlation identifier, per-requester rate-limit envelope, per-request minimization guidance, profile-combination policy) and four candidate primitives under evaluation (standard optional `correlation_token` request field, per-requester rate-limit headers, server-internal correlation join, hybrid). None are endorsed in `0.1.0-draft`. The pre-existing `### Cross-profile inference` subsection under Profile Boundary is now a forward link to the new top-level entry.
- New `docs/implementers/cross-profile-inference.md` worked attack-and-defense example. Synthetic two-call attack (commerce + boarding-preparation under the same authenticated principal) reconstructs a behavioral signature for an allergy-related condition without ever requesting a diagnosis scope; the doc walks through the layered defenses (per-request minimization, correlation-aware logging, per-requester rate limits across profiles, higher-scrutiny review of profile combinations, omission hygiene, freshness rounding) that defeat it. All identifiers are synthetic.
- `THREAT_MODEL.md` §Cross-Profile Inference tightened: converts the open-ended "Future drafts should define stronger guidance" line into a concrete future-draft agenda item tracked in GitHub issue #7 with forward links to the new compatibility-risks section and worked example.
- `docs/implementers/conformance-checklist.md` §Cross-enum and cross-profile boundaries expanded with five self-attestation entries covering per-profile-narrowness non-reliance, cross-profile per-requester rate limits, correlation-aware authorization logging, per-request minimization, and higher-scrutiny review of cross-profile grant sets.
