...
- `THREAT_MODEL.md` §Cross-Profile Inference expanded to frame cross-profile inference as a residual risk the stable line is not expected to close, with four open sub-decisions (correlation identifier, per-requester rate-limit envelope, per-request minimization guidance, profile-combination policy) and four candidate primitives under evaluation (standard optional `correlation_token` request field, per-requester rate-limit headers, server-internal correlation join, hybrid). None are endorsed in `0.1.0-draft`, and the work is tracked in GitHub issue #7 as ongoing research into residual-risk mitigations rather than as a decision required before `1.0`. The `compatibility-risks.md` §Profile Boundary subsection is a forward link to the threat-model entry.
