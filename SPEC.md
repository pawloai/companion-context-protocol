...
- Servers MUST NOT rely on per-profile narrowness alone to prevent inference across multiple authorized calls. A requester holding grants for more than one profile, or making repeated calls within a single profile, can correlate omissions, partial responses, freshness timestamps, and summaries to reconstruct restricted context that no single response would have disclosed. Servers granting multi-profile access (or repeat single-profile access) for the same requester SHOULD apply per-requester rate limits across all authorized profiles, correlation-aware authorization logging with enough metadata for retrospective abuse review, and per-request minimization (return only fields the declared purpose needs, even when the granted scopes would allow more). These mitigations are server-side policy responsibilities, not schema constraints; the canonical schemas cannot detect cross-call correlation. Cross-profile inference is a residual risk the stable line is not expected to fully close; see `THREAT_MODEL.md` §Cross-Profile Inference for the open sub-decisions (correlation identifier, rate-limit envelope, minimization guidance, profile-combination policy) and candidate primitives under ongoing research, and `docs/implementers/cross-profile-inference.md` for a worked attack-and-defense example.

The non-schema-checkable requirements above (principal binding, no-principal rejection, grantor-binding-at-issuance, vet rejection, cross-profile correlation policy) cannot be exercised by the canonical conformance runner because CCP is transport-neutral or runtime-policy-scoped. `docs/implementers/conformance-checklist.md` separates the machine-checked surface from the requirements that implementers MUST self-attest in their own integration tests or runtime authorization layer.
