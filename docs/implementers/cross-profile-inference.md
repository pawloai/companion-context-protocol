...
A synthetic adversary is an `agent_client` operating on behalf of a third-party meal-planner integration. The adversary's goal is to determine whether the pet is being treated for an allergy-related condition, without ever requesting a diagnosis, treatment, or wellness scope. The integration holds two grants for the same synthetic pet (`pet_xyz_synthetic`):
